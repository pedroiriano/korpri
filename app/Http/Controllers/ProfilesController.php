...
class ProfilesController extends Controller
{
    ...
    public function organization()
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://cms.depok.go.id/ViewPortal/get_content?siteId='.config("constants.siteId").'&status=ST01&kanalType=K006&limit=1');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0);
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 0);
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10);
        ...
        if ($response === false)
            $response = curl_error($ch);
        curl_close($ch);
        $diagram = json_decode($response, TRUE);

        return view('pages.profiles.organization', compact('organization', 'diagram'));
    }
